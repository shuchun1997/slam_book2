...
int main(int argc, char **argv) {
    ...
    Mat ref_depth;
    //将路径值传入readDatasetFiles中，以传参的形式将相对完整的路径和位姿传出来
    bool ret = readDatasetFiles(argv[1], color_image_files, poses_TWC, ref_depth);
    if (ret == false)
    {
        cout << "Reading image files failed!" << endl;
        return -1;  //查查return的返回值,负数应该是停止
    }
    ...
    Mat depth(height, width, CV_64F, init_depth);             // 深度图,深度图， 64F代表每一个像素点元素占64位浮点数，通道数为1
    ...
    for (int index = 1; index < color_image_files.size(); index++) {
        ...
        //通过当前图像更新深度滤波器，参数分别为参考图像（第一帧），当前图像，两者之间的位姿变换，深度、以及深度的方差（即不确定性的方差）
        //在此函数中调用updateDepthFilte来更新滤波器
        update(ref, curr, pose_T_C_R, depth, depth_cov2);//更新深度估计
        evaludateDepth(ref_depth, depth);  //评估深度估计
        //显示一下深度图（这个深度图不是通过RGBD相机拍摄的，而是通过三角测量以及深度滤波器的方式算出每个像素点的深度的）
...
bool readDatasetFiles(
    ...
    cv::Mat &ref_depth)
    {
    ifstream fin(path + "/first_200_frames_traj_over_table_input_sequence.txt");//数据集文件的路径
    if (!fin)
        return false;

    while (!fin.eof())   //eof()函数判断文件是否为空
    {
        // 数据格式：图像文件名 tx, ty, tz, qx, qy, qz, qw ，注意是 TWC 而非 TCW
        string image;
        fin >> image;
        double data[7];//存放fin读取的文件的数据
    ...
    // load reference depth  装载每个像素点的深度值
...
bool epipolarSearch(
    ...
    epipolar_direction.normalize();  //极线方向归一化??
