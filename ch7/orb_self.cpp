 ...
 * empty
 */
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors); //函数定义
 ...
 */
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches); //函数定义
...
int main(int argc, char **argv)
{
  ...
  assert(first_image.data != nullptr && second_image.data != nullptr);  //断言函数,相当于if语句

  // detect FAST keypoints1 using threshold=40
  chrono::steady_clock::time_point t1 = chrono::steady_clock::now();
  vector<cv::KeyPoint> keypoints1;   //<cv::KeyPoint>表示容器所装的数据类型
  ...
  //特征提取效果图1
  cv::Mat outimg1;
  drawKeypoints(first_image, keypoints1, outimg1);
  imshow("ORB features1", outimg1);
  //特征提取效果图2
  cv::Mat outimg2;
  drawKeypoints(second_image, keypoints2, outimg2);
...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors)
{
  ...
  const int half_patch_size = 3;   //应该就是fast焦点提取是的半径
  const int half_boundary = 16;    //半径为3圆上的16个像素点.
  int bad_points = 0;
